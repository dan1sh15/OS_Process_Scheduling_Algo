...
void computeCompletionTime(vector<vector<int>> &processes, int* CT, int n, vector<pair<int, bool>> &RT, vector<string> &ganttChart){
    priority_queue<pair<int,pair<int,int>>, vector<pair<int,pair<int,int>>>, greater<pair<int,pair<int,int>>>> rq; // create a minheap {BT, AT, id}
    ...
    while(i < n){
        ...
        if(!rq.empty()){ // IF READY QUEUE IS NOT EMPTY
            ...
            if(!RT[top.second.second].second){ // TO CHECK WHEN THE PROCESS ARRIVES FIRST
                RT[top.second.second].first = currTime - top.second.first; // calculate the response time since,  RESPONSE_TIME = CURRENT_CPU_TIME - ARRIVAL_TIME
            ...
            else{ // ELSE IF BURST TIME IS REMAINING THEN PUSH IT INTO THE READY QUEUE AGAIN
    ...
    while(!rq.empty()){ // THIS LOOP PERFORMS THE SAME LOGIC WHEN ALL THE PROCESSES ARRIVE IN THE READY QUEUE
        ...
        if(!RT[top.second.second].second){
        ...
        // SINCE ALL THE PROCESS HAVE ARRIVED IN THE READY QUEUE THEN WE CAN ADD THE BURST TIME DIRECTLY OF THE FEASIBLE PROCESS
        currTime += top.first;
        CT[top.second.second] = currTime;
    }
    return ;
}
...
The Average Turn Around Time is: 7.16667
...
The Gantt Chart for the above process is:
|P1|P1|P1|P1|P1|P1|P1|P1|P1|P1|P1|P1|P1|P1|P1|P1|P1|P1|P1|P1|P2|P2|P2|P2|P2|P2|P2|P2|P2|P2|P3|P3|P3|P3|P3|P3|P3|P3|P3|P3|P2|P2|P2|P2|P2|P2|P2|P4|
...
The Average Turn Around Time is: 23.75

The Average Waiting Time is: 6.25
